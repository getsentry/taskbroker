...
pub struct TopicConfig {
    ...
    /// If true, this topic is ONLY for producing (so for retries).
    ...
    /// The Kafka session timeout in milliseconds for this topic's consumer.
    /// Falls back to the global `kafka_session_timeout_ms` when unset.
    #[serde(default)]
    pub session_timeout_ms: Option<usize>,

    /// The interval in milliseconds at which this topic's consumer auto-commits.
...
impl ClusterConfig {
    ...
    /// Apply this cluster's `bootstrap.servers` and any configured SASL/SSL authentication
    /// onto a `ClientConfig` from the `rdkafka` crate. Shared by the consumer, producer and
