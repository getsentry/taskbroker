...
impl InflightActivation {
    /// Compute bucket from activation ID (UUID as number % 256). Used once on ingestion.
    pub fn bucket_from_id(id: &str) -> i16 {
        Uuid::parse_str(id)
            .map(|u| (u.as_u128() % 256) as i16)
            .unwrap_or(0)
    }
...
pub trait InflightActivationStore: Send + Sync {
    ...
    /// Only activations with bucket in [bucket_range.0, bucket_range.1] (inclusive) are returned.
